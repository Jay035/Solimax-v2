  ...
  return (
    <div className={className}>
      <label
        htmlFor={id}
        className={`${
          pathname === "/bridge" && !showModal && "flex justify-between items-center text-[#A4D0F2]"
        }`}
      >
        {label}
        {isRequired && <span className="text-[#F04438]">*</span>}
        {pathname === "/bridge" && !showModal && <span>Balance: {balance} SLM</span>}
      </label>
          ...
          {id === "amount" && (
            <p
              className={`text-[#A4D0F2] tracking-[-0.00875rem] text-sm absolute right-[1.18rem] top-[0.88rem] ${
                pathname === "/bridge" &&
                "w-[2.7rem] rounded-[1.25rem] text-center text-[#F4F4F5] bg-gradient-to-b from-[#6b03b8]/90 to-[#a818ba]/90 via-[#e22bbb]/90"
              }`}
            >
              MAX
            </p>
