...
export default function Container({}: Props) {
	...
	const [bridgingChain, setBridgingChain] = useState(56);
	const provider = useEthersProvider({ chainId: bridgingChain });
	...
	const [slmBalance, setSlmBalance] = useState(0);
	...
	const Bridger = async (bridgeObject: BridgeContract, toChain: string) => {
		...
		if (allowanceIsSatisfied) {
			return await bridgeObject
				.LockToken(amount.toString(), toChain)
				.then((res) => {
			...
			return await bridgeObject
				...
				.then(async (res) => {
					...
					await bridgeObject
						...
						.then((res) => {
							toast.success(
								"Token is Bridging..kindly wait" + res.hash
							);
							return res;
						})
						.catch((err) => {
        ...
        <form className="flex flex-col gap-6">
          <div className="flex gap-2 md:items-center ml-auto w-fit">
            ...
            /> */}
					...
					<div className="flex flex-col gap-[0.62rem]">
						<h3 className="text-transparent hidden md:inline">
							{" "}
							from
						</h3>
            ...
            /> */}
				...
				{isConnected ? (
					<button
						onClick={async () => {
							const txIndentifier =
								time.toString() + "-" + toChain.toString();
							// console.log({ txIndentifier });
							// console.log({ bridgingChain, recieveAmount });

							await BridgerMap[bridgingChain.toString()](
								txIndentifier
							);
						}}
